...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        team_blue.save()
        for user in users:
            team_blue.members.add(user)

        team_gold = Team(_id=ObjectId(), name='Gold Team')
        team_gold.save()
        for user in users[:3]:
            team_gold.members.add(user)
        ...
        self.stdout.write(self.style.SUCCESS('Successfully populated the database with test data for users, teams, activity, leaderboard, and workouts.'))
